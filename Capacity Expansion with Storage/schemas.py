...
class ExistingStorage(BaseModel):
    name: str
    technology: str                            # PHS, BESS, Hydrogen
    power_capacity_mw: float = Field(..., ge=0)
    energy_capacity_mwh: float = Field(..., ge=0)
    round_trip_efficiency: float = Field(..., gt=0, le=1.0) # e.g. 0.85
    vom_cost_per_mwh: float = Field(default=0.0, ge=0)       # $/MWh discharge
    initial_soc_mwh: float = Field(default=0.0, ge=0)

class CandidateStorage(BaseModel):
    name: str
    technology: str                            # PHS, BESS, Hydrogen
    annual_capex_per_mw: float = Field(..., ge=0)   # $/MW/year (Inverter/BOP)
    annual_capex_per_mwh: float = Field(..., ge=0)  # $/MWh/year (Cell/Reservoir)
...
class ExpansionInput(BaseModel):
    ...
    existing_storage: List[ExistingStorage] = Field(default_factory=list)
    candidate_storage: List[CandidateStorage] = Field(default_factory=list)
...
class ExpansionResults(BaseModel):
    ...
    new_storage_mw: Dict[str, float] = Field(default_factory=dict)   # Νέα ισχύς αποθήκευσης (MW)
    new_storage_mwh: Dict[str, float] = Field(default_factory=dict)
